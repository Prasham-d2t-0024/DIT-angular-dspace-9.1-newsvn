...
export class HomeTrendingItemComponent {
  ...
  this.chartService.findAllByGeolocation(apiCall).subscribe({
    next: (data) => {
      this.isLoading = false;

      if (data && data['_embedded']?.items) {
        this.cardData = data['_embedded'].items.map((item: any) => {
          return {
            ...item,
            casted$: this.castingObject(item), // cache observable once
            downloadData: this.transformChartData(item.itemCardData?.downlodeDatapoint),
            viewData: this.transformChartData(item.itemCardData?.viewDataPoint),
          };
        });
      }

      this.cdRef.detectChanges(); // trigger change detection after data update
    },
    error: (err) => {
      this.isLoading = false;
