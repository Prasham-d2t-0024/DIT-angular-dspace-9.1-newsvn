...
export class ItemComponent implements OnInit {
  ...
  Citation(content) {
    this.itemDataService.getCitetion(this.object.id).pipe().subscribe((data) => {
      if(!!data && data.statusCode === 200 && !!data.payload) {
        this.citation = Object.entries(data.payload);
        console.log("ITEM PAGE CITATION",this.citation);
      }
    });
  ...
  dowloadFile() {
    let URL =this.itemDataService.downloadCitetion(this.object.id,this.citationselection);
