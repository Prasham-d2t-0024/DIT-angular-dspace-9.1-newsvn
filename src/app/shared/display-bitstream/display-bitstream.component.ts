...
export class DisplayBitstreamComponent implements OnInit, AfterViewInit {
  ...
  getBistreamPAth(bistremobj: Bitstream) {
    bistremobj.format.pipe(
      getFirstSucceededRemoteData(),
      getRemoteDataPayload(),
    ).subscribe((format) => {
      if ((format.mimetype === 'application/pdf')) {
        this.pdfviwerhide = false;
        this.player4 = false;
        this.player3 = false;
        this.cdRef.detectChanges();
        this.auth.getShortlivedToken().pipe(take(1), map((token) =>
          hasValue(token) ? new URLCombiner(decodeURI(bistremobj._links.content.href), `?authentication-token=${token}`).toString() : decodeURI(bistremobj._links.content.href))).subscribe((logs: string) => {
            this.printFile = logs;
          ...
          this.downloadFile(bistremobj);
        }
      //  if ((format.mimetype === 'audio/mpeg')) {
      //   this.isLoading = true;
      //   const token: AuthTokenInfo = this.auth.getToken();
      //   let authorization = this.auth.buildAuthHeader(token);
      //   fetch(decodeURI(bistremobj._links.content.href), {
      //     method: "GET",
      //     headers: {
      ...
      //       this.pdfviwerhide = true;
      //       this.player4 = false;
      //       this.player3 = true;
      //       this.filePath = URL.createObjectURL(blob); // bind blob URL to video
      ...
      //     });
      // } else if ((format.mimetype === 'video/mp4' || format.mimetype ==='application/octet-stream')) {
      //   this.isLoading = true;
      //     const token: AuthTokenInfo = this.auth.getToken();
      //      let authorization = this.auth.buildAuthHeader(token);
      //     fetch(decodeURI(bistremobj._links.content.href), {
      ...
      //       });
      // } else {
      //   this.bitstreamDataService.getFilePathAndExtension(bistremobj.id).pipe(
      //  take(1),
      ...
      //     this.player3 = false;
      //     let url = ""
      //     if (response.FileExtension === 'svg' || response.FileExtension === 'heic' || response.FileExtension === 'heif' || response.FileExtension === 'webp' || response.FileExtension === 'tif' || response.FileExtension === 'bmp' || response.FileExtension === 'png' || response.FileExtension === 'jpeg' || response.FileExtension === 'tiff' || response.FileExtension === 'gif' || response.FileExtension === 'jpg') {
      //       url = 'http://202.21.38.245:8081/mira/?filename=' + response.FilePath.split('/assetstore/')[1] + '&filetype=' + response.FileExtension;
      //     } else {
      //       url = 'http://202.21.38.245:8081/documents/view?filename=' + response.FilePath.split('/assetstore/')[1] ;
      //     }
      //      this.auth.getShortlivedToken().pipe(take(1), map((token) =>
      //     hasValue(token) ? new URLCombiner(decodeURI(bistremobj._links.content.href), `?authentication-token=${token}`).toString() : decodeURI(bistremobj._links.content.href))).subscribe((logs: string) => {
  ...
  downloadFile(bistremobj: Bitstream) {
    const urlTree = this.router.createUrlTree([getBitstreamDownloadRoute(bistremobj)]);
